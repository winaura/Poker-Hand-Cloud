...
namespace EnhancedScrollerDemos.SnappingDemo
{
    public class SnappingDemo : MonoBehaviour
    {
        ...
        public int[] slotsInt;
        public MoneyFromSpin[] moneyRules;
        /// <summary>
        ...
        void UpdateInfo()
        {
            ...
            foreach (var slotController in _slotControllers)
            {
                slotController.Reload(slotsInt, _rarity);
            }
        }
        ...
        /// </summary>
        public int GetScore() => slotsInt[_snappedDataIndices[0]];
        public void SetRarity(RarityMenuButtons choosenRarity)
        {
            _rarity = choosenRarity;

            for (int i = 0; i < slotsInt.Length; i++)
            {
                if (_rarity != RarityMenuButtons.Lucky)
                {
                    slotsInt[i] = moneyRules[(int)_rarity].slotMoney
                     [(int)Choose(new float[] { 0.175f, 0.15f, 0.15f, 0.125f, 0.125f, 0.125f, 0.1f, 0.05f })];
                }

                else 
                {
                    slotsInt[i] = moneyRules[(int)_rarity].slotMoney
                     [(int)Choose(new float[] { 0.25f, 0.2f, 0.17f, 0.12f, 0.12f, 0.05f, 0.05f, 0.03f, 0.01f})];
                }
            }
